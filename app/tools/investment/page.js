...
import { ArrowLeft, Globe, TrendingUp, Info, Calculator, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function InvestmentCalculator() {
  ...
  const calculateInvestment = () => {
    ...
    // Interesse sul capitale iniziale
    let simpleInterestTotal = principal * rate * period;
    
    // Aggiungi interesse su ogni versamento ricorrente
    // Ogni versamento genera interesse solo per il tempo rimanente
    ...
    // Per versamenti ricorrenti intra-anno, usiamo la formula del valore futuro
    // con contributi periodici composti
    let compoundTotal = principal;
    
    // Tasso periodico (mensile, settimanale, ecc.)
    const periodicRate = rate / depositsPerYear;
    const totalPeriods = depositsPerYear * period;
    
    // Applica interesse composto sul capitale iniziale
    compoundTotal = principal * Math.pow(1 + periodicRate, totalPeriods);
    
    // Aggiungi i versamenti ricorrenti con interesse composto
    // Ogni versamento viene composto per i periodi rimanenti
    ...
    for (let year = 0; year <= period; year++) {
      // INTERESSE SEMPLICE per ogni anno
      let simpleYearInterest = principal * rate * year;
      const depositsUpToYear = depositsPerYear * year;
      
      // Aggiungi interesse su ogni versamento fino a questo anno
        ...
        <div class="chart-container">
          <div class="chart-legend">
            <div class="legend-item">
              ...
              <span>${t.results.chart.simpleInterest}</span>
            </div>
            <div class="legend-item">
              <div class="legend-color legend-compound"></div>
              <span>${t.results.chart.compoundInterest}</span>
            </div>
          </div>
                ...
                <div className="w-full">
                  ...
                  >
                    ...
                    {chartData.map((data, index) => {
                      ...
                      // Determina quali etichette mostrare in modo più intelligente
                      let showLabel = false;
